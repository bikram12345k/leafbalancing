...
##  This code is almost identical to the normal simulation code,
##	Except for line 228 specifying a laplace outcome model.
...
for(iter2 in 1:12){	# 12 iterations
  ...
  res <- foreach(j=1:length(taus), .combine =c, .packages = c('sensitivitymult','sensitivitymv','senstrat', 'MASS', 'rmutil')) %do% {
	  ...
	  ## Simulate outcome: 	
	  mydata$y<-rlaplace(n,m=taui*mydata$tr+mydata$x1+mydata$x2^2,s=1/sqrt(2))
  ...
  write.table(t(c(iter*900+iter2, res_tab)), 'sim_10k_laplace_6.csv', row.names = FALSE, col.names = FALSE, append = TRUE, sep=',') 
}  
